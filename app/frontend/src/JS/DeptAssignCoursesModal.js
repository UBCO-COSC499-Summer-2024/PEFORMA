...
const AssignCoursesModal = (props, courseData) => {
    let i = 0;
    ...
    const onSearch = (newSearch) => {
      ...
      const toggleCourseAssigned = (id, assign) => {
        
         for (let i = 0; i<props.courseData.courses.length;i++) {
             if (props.courseData.courses[i].id === id) {
                 if (!assign) {
                     props.courseData.courses[i].assigned = true;
                     setSearch(search);
                     forceUpdate();
                 } else {
                     props.courseData.courses[i].assigned = false;
                     setSearch(search);
                     forceUpdate();
                 }
                ...
                <tbody>
                    {currentCourses.map(course => {
                        ...
                        <tr key={i} className="instructor-item">
                            <td className='bold'>{course.courseCode}:</td><td>{course.title}</td>
                            <td>
                                <button id={course.id} className={"bold "+(course.assigned?"remove":"add")} onClick={() => toggleCourseAssigned(course.id, course.assigned)}>
