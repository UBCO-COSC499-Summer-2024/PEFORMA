...
const AssignRolesModal = (props, roleData) => {
    let i = 0;
    ...
    const onSearch = (newSearch) => {
      ...
      const toggleRoleAssigned = (id, assign) => {
        
         for (let i = 0; i<props.roleData.roles.length;i++) {
             if (props.roleData.roles[i].id === id) {
                 if (!assign) {
                     props.roleData.roles[i].assigned = true;
                     setSearch(search);
                     forceUpdate();
                 } else {
                     props.roleData.roles[i].assigned = false;
                     setSearch(search);
                     forceUpdate();
                 }
